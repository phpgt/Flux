...
?>
<div id="search-results" data-flux="autocomplete-results update-inner" aria-live="polite">
	<?php if(trim($query) === ''): ?>
	<p>Enter a city or country name to search.</p>
	<?php else: ?>
	<p><?= count($matches) ?> <?= count($matches) === 1 ? 'city' : 'cities' ?> matching “<?= h($query) ?>”.</p>
	<?php if($matches): ?>
